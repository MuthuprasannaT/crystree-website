@extends('Layouts.app')

@section('content')

<div class="breadcrumb-wrapper z-index-common overflow-hidden" data-bg-src="{{ asset('assets/img/bg/ai-banner.webp')}}" loading="lazy">
        <div class="overlay"></div>
        <div class="container">
            <div class="breadcrumb-wrapper__content wow animate__fadeInUp" data-wow-delay="0.45s">
                <h1 class="breadcrumb-wrapper__title">AI Integration</h1>
                <div class="breadcrumb-wrapper__menu--wrap">
                    <ul class="breadcrumb-wrapper__menu">
                        <li class="breadcrumb-wrapper__menu--item"><a href="{{('home')}}">Home</a></li>
                        <li class="breadcrumb-wrapper__menu--item">AI Integration</li>
                    </ul>
                </div>
            </div>
        </div>
    </div>
    <section class="vs-blog--layout1 space space-extra-bottom" dir="ltr">
        <div class="container">
            <div class="row">
                <div class="col-lg-8">
                    <div class="vs-blog vs-blog--single">
                        <figure class="vs-blog__figure"><img class="vs-blog__img" src="{{ asset('assets/img/blog/ai-service.webp')}}" alt="Blog Image" loading="lazy">
                            <div class="overlay"></div>
                        </figure>
                        <div class="vs-blog__content">
                            <h3 class="vs-blog__title mb-2">AI Integration Services</h3>
                            <svg width="120" height="10" viewBox="0 0 120 10" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M0 2C0 0.89543 0.895431 0 2 0H59.1716C59.702 0 60.2107 0.210714 60.5858 0.585786L66.5858 6.58579C67.8457 7.84572 66.9534 10 65.1716 10H2C0.89543 10 0 9.10457 0 8V2Z" fill="#014E4E" />
                                <path d="M73.4142 3.41421C72.1543 2.15428 73.0466 0 74.8284 0H109.172C109.702 0 110.211 0.210714 110.586 0.585786L116.586 6.58579C117.846 7.84572 116.953 10 115.172 10H80.8284C80.298 10 79.7893 9.78929 79.4142 9.41421L73.4142 3.41421Z"
                                fill="#FACF61" />
                            </svg>
                            <p class="pt-40">Artificial Intelligence is transforming the way businesses operate, helping organizations automate tasks, improve efficiency, and make smarter decisions. At Crystree Solutions, we provide advanced AI integration services that seamlessly connect AI technologies with your existing business systems, websites, mobile applications, and software platforms to enhance productivity and accelerate growth. AI integration is commonly used to automate workflows, improve decision-making, and optimize business operations through intelligent systems.</p>
                            <h4 class="mt-4 mb-15">Smart AI Solutions for Modern Businesses</h4>
                            <p class="mb-20">Our AI integration services are designed to help businesses leverage the power of artificial intelligence without disrupting existing workflows. We develop intelligent solutions that analyze data, automate repetitive tasks, improve customer interactions, and provide valuable business insights.</p>
                             <p class="mb-20">Whether you're a startup, SME, or enterprise organization, we deliver customized AI solutions tailored to your industry requirements and business objectives.</p>
                         
                            <ul class="vs-list style2">
                                <li>AI-Powered Chatbot Integration</li>
                                <li>Business Process Automation</li>
                                <li>AI Virtual Assistant Development</li>
                                <li>Predictive Analytics Solutions</li>
                                <li>Machine Learning Integration</li>
                            </ul>
                            <div class="row pt-30">
                                <div class="col-md-6 mb-25">
                                    <div class="vs-blog__video">
                                        <div class="overlay"></div><img src="{{ asset('assets/img/blog/auto-ai.webp')}}" alt="video" loading="lazy"></div>
                                </div>
                                <div class="col-md-6 mb-25">
                                    <div class="vs-blog__video">
                                        <div class="overlay"></div><img src="{{ asset('assets/img/blog/ai-mechan.webp')}}" alt="video" loading="lazy"></div>
                                </div>
                            </div>

                            <h4 class="mt-4 mb-15">AI-Powered Business Transformation</h4>
                            <p class="mb-20">Modern businesses generate vast amounts of data every day. AI integration helps transform this data into actionable insights, enabling organizations to make informed decisions, automate operations, and improve customer experiences.</p>
                             <p class="mb-20">By integrating artificial intelligence into your business ecosystem, you can unlock new opportunities, increase efficiency, and stay ahead in a rapidly evolving digital landscape.</p>
 <!-- FAQ Section Start -->
<div class="row  mt-60 ">
    <div class="col-lg-11">
        <div class="title-area text-center mb-40">
            <h4 class="mt-4 mb-15">Frequently Asked Questions</h4>
            {{-- <h3 class="vs-blog__title mb-2"></h3> --}}
            {{-- <span class="sec-subtitle"></span> --}}
            {{-- <h2 class="sec-title">Website Development FAQ</h2> --}}
        </div>

        <div class="accordion style3" id="websiteFaq">

            <div class="accordion-item">
                <h2 class="accordion-header">
                    <button class="accordion-button " type="button"
                        data-bs-toggle="collapse"
                        data-bs-target="#faqOne">
What is AI integration?
                    </button>
                </h2>
                <div id="faqOne" class="accordion-collapse collapse show"
                    data-bs-parent="#websiteFaq">
                    <div class="accordion-body">
AI integration involves connecting artificial intelligence technologies with existing business systems, applications, or workflows to automate tasks and improve decision-making </div>
                </div>
            </div>

            <div class="accordion-item">
                <h2 class="accordion-header">
                    <button class="accordion-button collapsed" type="button"
                        data-bs-toggle="collapse"
                        data-bs-target="#faqTwo">
Can AI be integrated into existing software?
                    </button>
                </h2>
                <div id="faqTwo" class="accordion-collapse collapse"
                    data-bs-parent="#websiteFaq">
                    <div class="accordion-body">
Yes. AI solutions can be integrated into websites, mobile applications, CRM systems, ERP platforms, and custom software applications.
                    </div>
                </div>
            </div>

            <div class="accordion-item">
                <h2 class="accordion-header">
                    <button class="accordion-button collapsed" type="button"
                        data-bs-toggle="collapse"
                        data-bs-target="#faqThree">
Is AI integration suitable for small businesses?
                    </button>
                </h2>
                <div id="faqThree" class="accordion-collapse collapse"
                    data-bs-parent="#websiteFaq">
                    <div class="accordion-body">
Absolutely. AI can help small businesses automate repetitive tasks, improve customer service, and optimize business operations.
                    </div>
                </div>
            </div>

            <div class="accordion-item">
                <h2 class="accordion-header">
                    <button class="accordion-button collapsed" type="button"
                        data-bs-toggle="collapse"
                        data-bs-target="#faqFour">
How secure are AI-powered systems?
                    </button>
                </h2>
                <div id="faqFour" class="accordion-collapse collapse"
                    data-bs-parent="#websiteFaq">
                    <div class="accordion-body">
We implement industry-standard security practices to ensure data protection, privacy, and secure AI operations.
                    </div>
                </div>
            </div>

            <div class="accordion-item">
                <h2 class="accordion-header">
                    <button class="accordion-button collapsed" type="button"
                        data-bs-toggle="collapse"
                        data-bs-target="#faqFive">
Do you provide ongoing AI support?
                    </button>
                </h2>
                <div id="faqFive" class="accordion-collapse collapse"
                    data-bs-parent="#websiteFaq">
                    <div class="accordion-body">
Yes. We offer continuous monitoring, maintenance, optimization, and technical support for all AI integration projects.
                    </div>
                </div>
            </div>
       <div class="col-12 mt-4">
    <div class="vs-header__action d-flex justify-content-center">
        <a href="#websiteFaq" class="vs-btn vs-btn--gradient">
            Explore FAQ
            {{-- <i class="fa-solid fa-arrow-right"></i> --}}
        </a>
    </div>
</div>

        </div>
    </div>
      
</div>

<!-- FAQ Section End -->

                            <h4 class="mb-15 mt-5">Why Choose Crystree Solutions for AI Integration?</h4>
                            <p class="mb-20">At Crystree Solutions, we combine technical expertise, innovation, and business understanding to deliver AI solutions that generate measurable results. Our team focuses on building practical AI applications that solve real business challenges and support long-term growth.</p>
                            <ul class="vs-list style2">
                                <li>Experienced AI Specialists</li>
                                <li>Customized AI Strategies</li>
                                <li>Business-Oriented Solutions</li>
                                <li>Secure Integration Practices</li>
                                <li>Scalable AI Infrastructure</li>
                            </ul>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
                    <div class="sidebar-area sticky-top">
                        <div class="widget widget_block widget_services">
                            <h3 class="widget__heading">Our Services</h3>
                            <ul class="wp-block-categories-list wp-block-categories">
                                <li class="cat-item cat-item-17"><a href="{{ route('products.crm') }}">CRM Software</a></li>
                                <li class="cat-item cat-item-26"><a href="{{ route('products.brand') }}">Branding Solutions</a></li>
                                <li class="cat-item cat-item-24"><a href="{{ route('products.reel') }}">Reel Marketing</a></li>
                                <li class="cat-item cat-item-24"><a href="{{ route('products.email') }}">Email Marketing</a></li>
                                <li class="cat-item cat-item-24"><a href="{{ route('products.gbp') }}">Google Business Profile</a></li>
                            </ul>
                        </div>
                        <!-- <div class="widget widget_block widget_newsletter bg-theme-17" data-bg-src="assets/img/elements/vs-bg-blog-sidebar-ele1.svg">
                            <h3 class="widget__heading">News Letter</h3>
                            <form action="{{ route('products.crm') }}">
                                <div class="wp-block-search__inside-wrapper">
                                    <p class="text-white">Subscribe email and get recent news and updates or offers.</p>
                                    <input placeholder="Search Here..." type="text" name="search" id="wp-search">
                                    <button class="vs-btn vs-btn--style14">Subscribe</button>
                                </div>
                            </form>
                        </div> -->
                    </div>
                </div>
            </div>
        </div>
    </section>

@endsection